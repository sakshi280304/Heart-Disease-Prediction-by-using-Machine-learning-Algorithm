...
def predict():
    ...
    final_prediction = f"{random_disease} - High Risk of Heart Disease" if avg[0] > len(
        all_models) / 2 else "Low Risk of Heart Disease"
    personal_info = [name, email]
    # Prepare response
    responses = [input_data, final_prediction, personal_info, accuracy, random_disease]
